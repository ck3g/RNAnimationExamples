# React Native Animation examples

The source of animation examples covered in the following articles.

- [Introduction to animation in React Native](https://whatdidilearn.info/2019/05/18/introduction-to-animation-in-react-native.html)
- [More about React Native animations](https://whatdidilearn.info/2019/06/10/more-about-react-native-animations.html)


## Animation examples

### Motion

![Vertical Motion](./examples/images/01-vertical-motion.gif)

### Fade in / Fade out

![Fade in](./examples/images/03-fade-in.gif)
![Fade out](./examples/images/02-fade-out.gif)

### Scale

![Positive scale](./examples/images/04-positive-scale.gif)
![Negative scale](./examples/images/05-negative-scale.gif)

### Resize

![Increasing size](./examples/images/06-increasing-size.gif)
![Decreasing size](./examples/images/07-decreasing-size.gif)

### Rotate

![Rotate](./examples/images/08-rotate.gif)
![Rotate X](./examples/images/09-rotate-x.gif)
![Rotate Y](./examples/images/10-rotate-y.gif)

### Change color

![Change color](./examples/images/11-change-color.gif)

### Spring

![Spring](./examples/images/12-spring.gif)

### Loop

![Loop](./examples/images/13-loop.gif)

### Decay

![Decay](./examples/images/14-decay.gif)

### Easing bounce

![Easing bounce](./examples/images/15-easing-bounce.gif)

### Sequence

![Sequence](./examples/images/16-sequence.gif)

### Parallel

![Parallel](./examples/images/17-parallel.gif)

### Delay

![Delay](./examples/images/18-delay.gif)

### Stagger

![Stagger](./examples/images/19-stagger.gif)


## License

[MIT](LICENSE)
